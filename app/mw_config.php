...
class mw_config
{
    ...
    static function handle_update_version_process()
    {
        ...
        if($olddbversion < MW_MIHANPANEL_DB_VERSION)
        {
          ...
          if ($olddbversion < 4)
          {
              ...
              {
                  mw_options::set_account_activation_type('link');
                  delete_option('mp_send_activation_link');
              }else{
                  mw_options::set_default_activation_account_type();
              }
          }
          if($olddbversion < 5)
          {
    ...
    {
        add_shortcode('mihanpanel', ['\mwplite\app\mw_shortcode', 'panel']);
        // WooCommerce Orders Shortcode
        add_shortcode('mp_woocommerce_orders', ['\mwplite\app\mw_shortcode', 'woocommerce_order']);

        if (!\mwplite\app\mw_tools::is_plugin_active('easy-digital-downloads/easy-digital-downloads.php')) {
        ...
        add_shortcode( 'mihanpanel_woo_downloads', ['\mwplite\app\mw_shortcode', 'woocommerce_downloads'] );
    }
    ...
    {
        $login_url = mw_options::get_login_url('?action=register');
        return $login_url;
    }
    static function handle_after_register_message($errors)
    {
        $activation_account_mode = mw_options::get_account_activation_type();
        if(isset($errors->errors['registered']) && ($activation_account_mode == mw_options::AUTO_ACTIVATION_MODE || $activation_account_mode == mw_options::MANUAL_ACTIVATION_MODE))
        {
            switch($activation_account_mode)
            {
                case mw_options::AUTO_ACTIVATION_MODE:
                    $new_message = __("Registration complete. Your account has been activated automatically.", 'mihanpanel');
                break;
                case mw_options::MANUAL_ACTIVATION_MODE:
    ...
    public static function user_account_activation_process()
    {
        ...
        $validate_res = mw_users::validate_activation_code($activation_code);
        $state = $validate_res ? 'completed' : 'error';
        wp_redirect(mw_options::get_login_url('?activation_status=' . $state));
        exit();
    }
    public static function account_activation_message_handler($message)
    {
        ...
        {
            $activation_account_mode = mw_options::get_account_activation_type();
            if ($activation_account_mode === mw_options::MANUAL_ACTIVATION_MODE)
            {
    ...
    {
        if (mw_users::get_activation_code($user->id))
        {
            wp_logout();
            wp_redirect(mw_options::get_login_url('?action=inactive_account'));
            exit();
        }
    }
    static function redirect_non_admin_after_login($user_login, $user)
    {
        if(mw_users::is_admin_user())
        {
            return false;
        }
    ...
    public static function redirect_non_admin_from_admin_panel()
    {
        ...
        wp_safe_redirect(mw_options::get_panel_url());
        exit();
    }
    static function redirect_to_option_panel_handler()
    {
        if(\mwplite\app\mw_config::check_for_redirect_to_option_panel())
            {
                \mwplite\app\mw_config::unset_redirect_to_option_panel();
