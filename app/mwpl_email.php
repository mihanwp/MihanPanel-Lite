...
if(defined('ABSPATH') && !class_exists('mwpl_email'))
{
    class mwpl_email
    {
        ...
        {
            $email_content = mwpl_options::get_email_notify_content();
        ...
        {
            $title = mwpl_options::get_reset_password_email_subject();
        ...
        public static function filter_reset_password_email_message($message, $key, $user_login, $user_data)
        {
            ...
            $content = mwpl_options::get_reset_password_email_content();
            if (!$content)
                return $message;
    
            $login_slug = mwpl_options::get_login_slug();
        ...
        {
            $activation_link = mwpl_users::get_new_activation_link($user_id);
        ...
        {
            $message = \mwplite\app\mwpl_options::get_change_account_status_email_content();
            if(!$message)
            {
                return false;
            }
            $subject = \mwplite\app\mwpl_options::get_change_account_status_email_subject();
            if(!$subject)
            {
                $blogname = wp_specialchars_decode( get_option( 'blogname' ), ENT_QUOTES );
